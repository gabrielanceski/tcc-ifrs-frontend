      ...
      <form @submit.prevent="submit">
        <div class="grid grid-cols-2 py-5 divide-x">
          ...
          <div class="px-20 flex flex-col">
            ...
            <select id="role" v-model="form.role" class="block w-full px-3 py-2 mt-1 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-sky-500 focus:border-sky-500 sm:text-sm mb-4">
              <option v-show="role.rolesCanManipulate.includes(currentUser?.role)" v-for="(role, index) in userRoles" :value="role.name" :key="index">{{role.label}}</option>
            </select>
            <label for="password" class="block text-sm font-medium text-gray-700">Senha</label>
            <Input id="password" type="password" placeholder="Crie uma senha" v-model="form.password" class="mb-2" />
            <label for="repeatPassword" class="block text-sm font-medium text-gray-700">Repita a Senha</label>
            <Input id="repeatPassword" type="password" placeholder="Repita a senha" v-model="repeatPassword" class="mb-2" />
